describe('withLatestFrom', function()
  it('returns the observable it is called on if no other sources are specified', function()
    local observable = Rx.Observable.fromRange(1, 5):withLatestFrom()
  ...
  it('should produce the most recent values when the first observable produces a value', function()
    ...
    local onNext = spy()
    subjectA:withLatestFrom(subjectB):subscribe(Rx.Observer.create(onNext))
    subjectA:onNext('a')
    subjectA:onNext('b')
    subjectB:onNext('c')
    subjectB:onNext('d')
  ...
  it('should complete only when the first observable completes', function()
    ...
    subjectA:withLatestFrom(subjectB):subscribe(Rx.Observer.create(_, _, onCompleted))
    subjectA:onNext('a')
    subjectB:onNext('b')
  ...
  it('should unsubscribe from all source observables', function()
    ...
    local subscription = observableA:withLatestFrom(observableB):subscribe()
